...
const createProgressTrackerMock = () => {
...
describe('message formatter using progress tracker', () => {
    it('returns message for given instance', () => {
        ...
        expect(message).toMatch(/Done: 30000 Rate: 240ops\/m, 14400ops\/h, 345600ops\/d Running: 59s Memory usage: \d+(\.\d+)? MB/);
    });

    it('return message for given instance with specified percentage precision', () => {
        const message = messageFormatter.format(createRateTrackerMock(), 3);

        expect(message).toMatch(/Done: 30000 Rate: 240\.000ops\/m, 14400\.000ops\/h, 345600\.000ops\/d Running: 59s Memory usage: \d+(\.\d+)? MB/);
    });
});
